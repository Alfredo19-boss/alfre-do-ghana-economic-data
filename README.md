...
- **Charts tab** (`#charts`): seventeen charts drawn from the dashboard's own figures — the debt by year, debt-to-GDP, debt per person, inflation since 1993, inflation against the policy rate, the cedi, real growth, income per person, reserves, remittances, unemployment, population, exports against imports, the trade balance, where the budget goes, African inflation and the largest African economies. They **slide one at a time** — arrows, dots, arrow keys or a swipe, with a slow auto-advance that stops the moment you touch anything; *Show all* puts the whole wall back. **Point at any chart or touch it** and a read-out follows your finger with the value under it, a crosshair on the lines and the bar lit up. Each chart also offers **Read the numbers** (the same data as a table), **Download PNG** (titled and sourced, ready to paste into a report) and **Download CSV**. The series colours are a colour-blind-checked set, assigned in a fixed order and never cycled
